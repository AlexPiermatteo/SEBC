# Installation MySQL
An instance of MySQL Server is installed on the first two nodes of the cluster (sebc1 and sebc2)
```
[ec2-user@ip-10-0-0-22 ~]$ wget http://repo.mysql.com/mysql-community-release-el7-5.noarch.rpm
[ec2-user@ip-10-0-0-22 ~]$ sudo rpm -ivh mysql-community-release-el7-5.noarch.rpm
[ec2-user@ip-10-0-0-22 ~]$ sudo yum update
[ec2-user@ip-10-0-0-22 ~]$ sudo yum install mysql-server -y
[ec2-user@ip-10-0-0-22 ~]$ sudo service mysqld start
```
To install the mysql client, on everynode the following commands were executed with ansible:
```
[ec2-user@ip-10-0-0-22 ~]$ wget http://repo.mysql.com/mysql-community-release-el7-5.noarch.rpm
[ec2-user@ip-10-0-0-22 ~]$ sudo rpm -ivh mysql-community-release-el7-5.noarch.rpm
[ec2-user@ip-10-0-0-22 ~]$ sudo yum update
[ec2-user@ip-10-0-0-22 ~]$ sudo yum install mysql
[ec2-user@ip-10-0-0-22 ~]$ sudo rm mysql-community-release-el7-5.noarch.rpm
```
## Configure MySQL Server
All the following commands need to be executed on both replicas, so on hosts sebc1 and sebc2. 
...
Copy and paste the following in the file `/etc/my.cnf` (it wasn't there, so I had to create one). Set the `server_id=1` in master (sebc1) and 2 in all slaves (sebc2).
```
[client]
port    	= 3306
...
```
[ec2-user@ip-10-0-0-22 ~]$ sudo service mysqld stop
Redirecting to /bin/systemctl stop  mysqld.service
[ec2-user@ip-10-0-0-22 ~]$ sudo rm /var/lib/mysql/ib_logfile*
[ec2-user@ip-10-0-0-22 ~]$ sudo service mysqld start
...
```
[ec2-user@ip-10-0-0-22 ~]$ mysql -u root -p
Enter password:
...
mysql> CHANGE MASTER TO MASTER_HOST='ip-10-0-0-22.eu-west-1.compute.internal',MASTER_USER='slave_user', MASTER_PASSWORD='password', MASTER_LOG_FILE='mysql-bin.000001', MASTER_LOG_POS=403;
